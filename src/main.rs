...
fn launch_control_thread<T, E: Debug>(
    ...
    real: bool,
    side: TeamColor,
    controller: impl RobotController<T, E> + Send + 'static,
) -> JoinHandle<()> {
    let mut interval = tokio::time::interval(CONTROL_PERIOD);
    tokio::spawn(async move {
        let mut controller = controller;
        let mut vision = Vision::new(vision_address, vision_port, real);
            ...
            for packet in vision.get_pending_packets().await {
                if let Some(detection) = packet.detection {
                    ...
                    for ally_detection in allies {
                        ...
                        let r = world.team.get_mut(&rid).unwrap_or(continue);
                        r.set_orientation(ally_detection.orientation());
                        r.set_pos(Point2::new(ally_detection.x, ally_detection.y));
                    }
